...
def plot_bboxes(image, boxes, masks, labels=[], colors=[], score=True, conf=None):
    # Define COCO Labels
    if labels == []:
        labels = {
            0: "__background__",
    ...
    for idx, box in enumerate(boxes):
        ...
        if conf:
            ...
            cv2.polylines(image, [segment], True, color, 4)
...
if __name__ == '__main__':
    ...
    im_input = cv2.imread(path)
    res = cls_model(im_input)

    res_img = plot_bboxes(im_input, res[0].boxes.data, res[0].masks)
